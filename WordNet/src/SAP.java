    ...
    private class BFS {
        ...
        int searchOfShortestPath() {
            ...
            while (!q1.isEmpty() || !q2.isEmpty()) {
                int dist = foundAncestor == -1 ? -1 : vertices.get(foundAncestor);

                int a1 = findShortestPath(q1, marked1, edgeTo1, distTo1, dist);

                if (-1 != a1 && -1 == foundAncestor)
                    dist = vertices.get(a1);
                if (-1 != a1 && -1 != foundAncestor && vertices.get(a1) < dist)
                    dist = vertices.get(a1);

                int a2 = findShortestPath(q2, marked2, edgeTo2, distTo2, dist);
                ...
                if (-1 != a2 && -1 == a1)
                    foundAncestor = a2;
                if (-1 != a1 && -1 != a2)
                    foundAncestor = vertices.get(a1) < vertices.get(a2) ? a1 : a2;
            }
            return foundAncestor;
        }
         ...
         * @return
         */
        int findShortestPath(Queue<Integer> q, boolean[] marked, int[] edgeTo, int[] distTo, int dist) {
            int pathFound = -1;
            ...
            for (int w : graph.adj(v)) {
                if (!marked[w]) {
                    ...
                    marked[w] = true;
                    q.enqueue(w);
                    if (vertices.containsKey(w) && (dist == -1 || dist > vertices.get(w) + distTo[w])) {
                        pathFound = w;
                        vertices.replace(w, vertices.get(w) + distTo[w]);
                    } else
                        vertices.put(w, distTo[w]);
                }
            }
            return pathFound;
        }
